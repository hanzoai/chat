#!/usr/bin/env bash
# Regression test for the chat ConfigMap template.
#
# Background: at one point templates/configmap.yaml ran
# `.Values.chat.configYamlContent | toYaml | indent 4`. Because the
# value is already a YAML literal string, toYaml re-wrapped it in another `|`
# block scalar, producing a mounted /app/chat.yaml whose first line was
# a bare `|`. js-yaml "recovered" by returning the body as a string, so
# Chat silently fell back to internal defaults for every config block
# (endpoints, interface, modelSpecs, ...).
#
# This test renders the ConfigMap with a sample configYamlContent and asserts
# that the rendered chat.yaml is a proper YAML object preserving nested
# keys.
...
VALUES_FILE="$(mktemp -t chat-configmap-values.XXXXXX)"
RENDERED_FILE="$(mktemp -t chat-configmap-render.XXXXXX)"
...
helm template chat "${CHART_DIR}" \
  --show-only templates/configmap.yaml \
  -f "${VALUES_FILE}" > "${RENDERED_FILE}"

# Pull the block scalar body that lives under `chat.yaml: |`.
...
if [[ -z "${BODY}" ]]; then
  echo "FAIL: could not extract chat.yaml body from rendered ConfigMap" >&2
  cat "${RENDERED_FILE}" >&2
  exit 1
fi
...
if [[ "${FIRST_LINE}" == "|" ]]; then
  echo "FAIL: rendered chat.yaml starts with a bare '|' — configYamlContent is double-wrapped" >&2
...
if ! printf '%s\n' "${BODY}" | grep -qE '^version: 1\.3\.11$'; then
  echo "FAIL: expected top-level 'version: 1.3.11' in rendered chat.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi

if ! printf '%s\n' "${BODY}" | grep -qE '^endpoints:$'; then
  echo "FAIL: expected top-level 'endpoints:' key in rendered chat.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi

if ! printf '%s\n' "${BODY}" | grep -qE '^  agents:$'; then
  echo "FAIL: expected nested 'endpoints.agents' key in rendered chat.yaml" >&2
  printf '%s\n' "${BODY}" >&2
  exit 1
fi
...
if not isinstance(doc, dict):
    sys.stderr.write(f"FAIL: parsed chat.yaml is {type(doc).__name__}, expected dict\n")
    sys.exit(1)
  ...
  node)
    BODY="${BODY}" node -e '
      ...
      if (doc === null || typeof doc !== "object" || Array.isArray(doc)) {
        console.error("FAIL: parsed chat.yaml is " + typeof doc + ", expected object");
        process.exit(1);
      }
...
echo "PASS: rendered chat.yaml is a proper YAML document with nested keys preserved"
