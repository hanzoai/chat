...
describe('Environment Variable Extraction (MCP)', () => {
  ...
  describe('processMCPEnv', () => {
    ...
    it('should ensure different users with same starting config get separate values', () => {
      ...
      expect(baseConfig.headers?.['User-Id']).toBe('{{CHAT_USER_ID}}');
    ...
    it('should process dynamic user fields in headers', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          ...
          'User-Name': '{{CHAT_USER_USERNAME}}',
          OpenID: '{{CHAT_USER_OPENIDID}}',
          'Google-ID': '{{CHAT_USER_GOOGLEID}}',
          'Email-Verified': '{{CHAT_USER_EMAILVERIFIED}}',
    ...
    it('should process user fields in env variables', () => {
      ...
      const options: MCPOptions = {
        ...
        args: ['server.js'],
        env: {
          USER_EMAIL: '{{CHAT_USER_EMAIL}}',
          LDAP_ID: '{{CHAT_USER_LDAPID}}',
          API_KEY: '${TEST_API_KEY}',
        },
      };
    ...
    it('should process user fields in URL', () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api/{{CHAT_USER_USERNAME}}/stream',
      };
    ...
    it('should handle boolean user fields', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'Email-Verified': '{{CHAT_USER_EMAILVERIFIED}}',
          'Two-Factor': '{{CHAT_USER_TWOFACTORENABLED}}',
          'Terms-Accepted': '{{CHAT_USER_TERMSACCEPTED}}',
        },
      };
    ...
    it('should not process sensitive fields like password', () => {
      ...
      const options: MCPOptions = {
        ...
        headers: {
          'User-Email': '{{CHAT_USER_EMAIL}}',
          'User-Password': '{{CHAT_USER_PASSWORD}}', // This should not be processed
        },
      };
      ...
      expect('headers' in result && result.headers).toEqual({
        'User-Email': 'test@example.com',
        'User-Password': '{{CHAT_USER_PASSWORD}}', // Unchanged
      });
    });
    ...
    it('should support both id and _id properties for CHAT_USER_ID', () => {
      ...
      expect('headers' in result2 && result2.headers?.['User-Id']).toBe('{{CHAT_USER_ID}}');
    ...
    it('should process customUserVars in URL field', () => {
      ...
      const options: MCPOptions = {
        type: 'websocket',
        url: 'wss://example.com/{{TENANT_ID}}/api/{{API_VERSION}}?user={{CHAT_USER_ID}}&key=${TEST_API_KEY}',
      };
    ...
    it('should prioritize customUserVars over user fields and system env vars if placeholders are the same (though not recommended)', () => {
      ...
      process.env.CHAT_USER_EMAIL = 'system-email-should-be-overridden';
      const user = createTestUser({ email: 'user-email-should-be-overridden' });
      const customUserVars = {
        CHAT_USER_EMAIL: 'custom-email-wins',
      };
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://example.com/api',
        headers: {
          'Test-Email': '{{CHAT_USER_EMAIL}}', // Placeholder that could match custom, user, or system
        },
      };
      ...
      // Clean up env var
      delete process.env.CHAT_USER_EMAIL;
    });
    ...
    it('should handle placeholders with no matching customUserVars (falling back to user/system vars)', () => {
      ...
      const options: MCPOptions = {
        ...
        url: 'https://example.com/api',
        headers: {
          'User-Email-Header': '{{CHAT_USER_EMAIL}}', // Should use user.email
    ...
    it('should correctly process a mix of all variable types', () => {
      ...
      const obj = {
        type: 'streamable-http' as const,
        url: 'https://{{CUSTOM_ENDPOINT_ID}}.example.com/users/{{CHAT_USER_USERNAME}}',
        headers: {
        ...
        env: {
          ...
          USER_HOME_DIR: '/home/{{CHAT_USER_USERNAME}}',
    ...
    it('should process GitHub MCP server configuration with PAT_TOKEN placeholder', () => {
      ...
      // Simulate the GitHub MCP server configuration from chat.yaml
      const options: MCPOptions = {
        type: 'streamable-http',
    ...
    it('should leave {{CHAT_GRAPH_ACCESS_TOKEN}} unchanged (resolved by preProcessGraphTokens)', () => {
  ...
  describe('preProcessGraphTokens and processMCPEnv working in tandem', () => {
    ...
    it('should resolve graph tokens in env and user placeholders in headers', async () => {
      ...
      const options: MCPOptions = {
        command: 'node',
        args: ['mcp-server.js', '--user', '{{CHAT_USER_USERNAME}}'],
        env: {
          GRAPH_ACCESS_TOKEN: '{{CHAT_GRAPH_ACCESS_TOKEN}}',
    ...
    it('should resolve graph tokens in URL alongside other placeholders', async () => {
      ...
      const options: MCPOptions = {
        type: 'sse',
        url: 'https://{{TENANT_ID}}.example.com/api?token={{CHAT_GRAPH_ACCESS_TOKEN}}&user={{CHAT_USER_ID}}',
      };
