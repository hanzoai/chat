...
describe('resolveHeaders', () => {
  ...
  it('should process CHAT_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{CHAT_BODY_CONVERSATIONID}}' };
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{CHAT_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should handle numeric values with body placeholders', () => {
      ...
      const headers = {
        ...
        'X-Conv': '{{CHAT_BODY_CONVERSATIONID}}',
      };
      const result = resolveHeaders({ headers, body });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{CHAT_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{CHAT_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{CHAT_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{CHAT_BODY_CONVERSATIONID}}',
        parent: '{{CHAT_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{CHAT_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{CHAT_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{CHAT_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        ...
        USER_ROLE: '{{CHAT_USER_ROLE}}',
      },
      args: ['--user', '{{CHAT_USER_USERNAME}}', '--id', '{{CHAT_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{CHAT_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{CHAT_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{CHAT_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{CHAT_USER_ID}} conversation:{{CHAT_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{CHAT_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{CHAT_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{CHAT_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{CHAT_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
