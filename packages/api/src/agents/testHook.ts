...
import type { Run, IState } from '@hanzochat/agents';
 ...
 * `CHAT_TEST_RUN_HOOK` points at a module, it is loaded and invoked with
...
export function applyTestRunHook(run: Run<IState>, context: TestRunHookContext): void {
  const hookPath = process.env.CHAT_TEST_RUN_HOOK;
  if (!hookPath) {
    return;
  }
  try {
