 ...
 *
 * These three exports were dropped by an upstream sync (`49dc4f7bf6`) while
 * `packages/api/src/admin/config.ts` and `packages/data-schemas/src/app/resolution.ts`
 * kept importing them, so `INTERFACE_PERMISSION_FIELDS.has(...)` ran against
 * `undefined` and the admin config surface threw on every override write.
 */
