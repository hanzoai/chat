...
/**
 * Imports a Chat conversation from JSON.
 *
 ...
 * @returns {Promise<void>} - A promise that resolves when the import is complete.
 */
async function importChatConvo(
  jsonData,
  requestUserId,
  builderFactory = createImportBatchBuilder,
) {
  try {
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Chat file`, error);
  }
}
