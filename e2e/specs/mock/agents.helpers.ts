...
import type { GraphEdge } from '@hanzochat/data-provider';
...
export async function findAgent(
  ...
  const body = await fetchJson<{ data?: AgentSummary[] }>(
    page,
    `/v1/chat/agents?search=${encodeURIComponent(agentName)}&limit=10&requiredPermission=${AGENT_EDIT_PERMISSION}`,
    token,
  );
  ...
  for (let attempt = 0; attempt < 20; attempt++) {
    ...
    if (agent) {
      latestAgent = await fetchJson<AgentDetail>(
        page,
        `/v1/chat/agents/${encodeURIComponent(agent.id)}/expanded`,
        token,
      );
...
export async function cleanupAgent(page: Page, agentId?: string) {
  ...
  await requestJson<{ message?: string }>(page, {
    path: `/v1/chat/agents/${encodeURIComponent(agentId)}`,
    token,
    method: 'DELETE',
  });
}
