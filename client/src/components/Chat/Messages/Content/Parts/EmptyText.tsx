  ...
  return (
    <div className="text-message mb-[0.625rem] flex min-h-[20px] flex-col items-start gap-3 overflow-visible">
      <div className="markdown prose dark:prose-invert light w-full break-words dark:text-gray-100">
        {/* The dot draws itself out of flow, so it needs the positioned box;
            text wants the flow, so it wraps at the message width instead of
            running off the side. */}
        <div className={english ? undefined : 'absolute'}>
          <p className="submitting relative">
            {/* Empty, this span IS the dot — which is what a UI in a language
                these jokes were not written in still gets. */}
            <span className="result-thinking text-sm text-text-secondary">
              {english ? (
                <>
                  {/* A beat is a joke, not a status. Announcing each one would
                      read four fragments a second apart to a screen reader, so
                      it hears the plain word once and the eye gets the joke. */}
