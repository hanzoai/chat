...
import { isServedImage } from '@hanzochat/data-provider';
...
const ConvoIconURL: React.FC<ConvoIconURLProps> = ({
  ...
  const isURL = useMemo(
    () => !!(iconURL && (iconURL.includes('http') || isServedImage(iconURL))),
    [iconURL],
  );
  if (isURL) {
    return (
  ...
  return (
    <div className="shadow-stroke relative flex h-full items-center justify-center rounded-full bg-surface-tertiary text-text-primary">
      {Icon && (
        <Icon
          size={41}
          context={context}
